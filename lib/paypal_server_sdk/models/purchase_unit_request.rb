...
module PaypalServerSdk
  ...
  class PurchaseUnitRequest < BaseModel
    ...
    # This field supports up to 3,000 characters, but any content beyond 127
    # characters (including spaces) will be truncated. The 127 character limit
    # is reflected in the response representation of this field. The purchase
    # description. The maximum length of the character is dependent on the type
    # of characters used. The character length is specified assuming a US ASCII
    # character. Depending on type of character; (e.g. accented character,
    # Japanese characters) the number of characters that that can be specified
    # as input might not equal the permissible max length.
    # @return [String]
    ...
    # receives. invoice_id values are required to be unique within each merchant
    # account by default. Although the uniqueness validation is configurable,
    # disabling this behavior will remove the account's ability to use
    # invoice_id in other APIs as an identifier. It is highly recommended to
    # keep a unique invoice_id for each Order.
    # @return [String]
    attr_accessor :invoice_id

    # This field supports up to 127 characters, but any content beyond 22
    # characters (including spaces) will be truncated. The 22 character limit is
    # reflected in the response representation of this field. The soft
    # descriptor is the dynamic text used to construct the statement descriptor
    # that appears on a payer's card statement. If an Order is paid using the
    # "PayPal Wallet", the statement descriptor will appear in following format
    ...
    def self.from_hash(hash)
      ...
      unless hash['items'].nil?
        ...
        hash['items'].each do |structure|
          items << (ItemRequest.from_hash(structure) if structure)
        end
      end
